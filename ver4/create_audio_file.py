...
try:
    ...
    try:
        with open("speed_audio_set.py", "r", encoding="cp1251") as f:
            exec(f.read())
    except UnicodeDecodeError:
        with open("speed_audio_set.py", "r", encoding="latin-1") as f:
            exec(f.read())
except Exception as e:
    print(f"Ошибка при запуске speed_audio_set.py: {e}")
