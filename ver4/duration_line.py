...
if final_task == 0:
    print(f"Значение переменной final_task: {final_task}")
    print("Запуск файла send_command_to_AhoTTS.py...")
    proc = subprocess.Popen(
        ["python", "send_command_to_AhoTTS.py"],
        creationflags=subprocess.CREATE_NEW_PROCESS_GROUP,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        stdin=subprocess.DEVNULL,  # <- игнорировать input
        text=True,
        encoding="utf-8"
    )
    ...
    print("Процесс send_command_to_AhoTTS.py завершился с кодом:", code)

    if not os.path.exists("out.wav") or os.path.getsize("out.wav") == 0:
    ...
    print("Запуск send_command_to_AhoTTS.py отменён")